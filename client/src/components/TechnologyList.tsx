...
export default function TechnologyList() {
  ...
  const filteredTechnologies = technologies
    ? technologies.filter((tech: Technology) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 container mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <h2 className="text-xl font-bold mb-2 md:mb-0">All Technologies</h2>
        <div className="w-full md:w-auto flex flex-col md:flex-row gap-2">
          ...
          <select 
            className="border border-slate-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Quadrant</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Ring</th>
          ...
          <div className="flex items-center justify-between mt-6">
            <div className="text-sm text-slate-500">
              {filteredTechnologies.length > 0 ? (
                <>
                  ...
                  of <span className="font-medium">{filteredTechnologies.length}</span> results
                </>
              ) : (
                "No results found"
              )}
            ...
            {totalPages > 1 && (
              <div className="flex items-center space-x-2">
                <button 
                  className="px-3 py-1 rounded border border-slate-300 bg-white text-slate-500 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                {totalPages > 5 && currentPage < totalPages - 2 && (
                  <>
                    ...
                    <button
                      className={`px-3 py-1 rounded border border-slate-300 bg-white text-slate-700 hover:bg-blue-50`}
                ...
                <button 
                  className="px-3 py-1 rounded border border-slate-300 bg-white text-slate-700 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
